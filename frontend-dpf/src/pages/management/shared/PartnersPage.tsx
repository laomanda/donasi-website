...
export function PartnersPage() {
  ...
  const goEdit = (id: number) => navigate(`${routeBase}/partners/${id}/edit`);
  ...
  const onDeleteSelected = async () => {
    ...
    try {
      const result = await runWithConcurrency(selection.selectedIds, 4, async (id) => {
        await http.delete(`${apiBase}/partners/${id}`);
      });
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-600">
              Kelola mitra yang tampil di halaman publik: logo, tautan, urutan, dan status aktif.
            </p>
          ...
          <button
            type="button"
            onClick={() => navigate(`${routeBase}/partners/create`)}
