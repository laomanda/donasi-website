...
import { faArrowLeft, faBuildingColumns, faTrash } from "@fortawesome/free-solid-svg-icons";
...
export function AdminBankAccountForm({ mode, accountId }: { mode: Mode; accountId?: number }) {
  ...
  const payloadForRequest = (state: BankAccountFormState) => {
    ...
    return {
      ...
      is_visible_public: Boolean(state.is_visible_public),
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        await http.post("/admin/bank-accounts", payload);
        toast.success("Rekening berhasil dibuat.", { title: "Berhasil" });
      } else {
        await http.put(`/admin/bank-accounts/${accountId}`, payload);
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <div className="mt-5 space-y-4">
              ...
              >
                ...
                <span className="text-xs font-semibold opacity-80">{form.is_visible_public ? "Aktif" : "Nonaktif"}</span>
              </button>
