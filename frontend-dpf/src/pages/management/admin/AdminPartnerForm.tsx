...
export function AdminPartnerForm({ mode, partnerId }: { mode: Mode; partnerId?: number }) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        await http.post("/admin/partners", payload);
        toast.success("Mitra berhasil dibuat.", { title: "Berhasil" });
      } else {
        await http.put(`/admin/partners/${partnerId}`, payload);
  ...
  const onDelete = async () => {
    ...
    setErrors([]);
    try {
      await http.delete(`/admin/partners/${partnerId}`);
