...
const getLocalDatetimeString = (dateInput?: string | Date | null) => {
...
export function AdminDonationManualEditPage() {
  ...
  // Base path based on current route (admin or keuangan)
  ...
  const canSubmit = !saving && !uploadingProof && !loading;
  ...
  useEffect(() => {
    if (!donationId || Number.isNaN(donationId)) {
      setErrors(["ID donasi tidak valid."]);
      setLoading(false);
      return;
    }
    ...
    setLoading(true);
    http
      .get(`/admin/donations/${donationId}`)
      .then((res) => {
        if (!active) return;
        const d = res.data;
        setDonationCode(d.donation_code || `#${d.id}`);
        setForm({
          program_id: d.program_id ? String(d.program_id) : "",
          donor_name: d.donor_name || "",
          donor_email: d.donor_email || "",
          donor_phone: d.donor_phone || "",
          amount: d.amount ? String(Math.round(Number(d.amount))) : "",
          is_anonymous: Boolean(d.is_anonymous),
          payment_method: d.payment_method || "Transfer Bank",
          payment_channel: d.payment_channel || "",
          notes: d.notes || "",
          paid_at: d.paid_at ? getLocalDatetimeString(d.paid_at) : getLocalDatetimeString(d.created_at),
          manual_proof_path: d.manual_proof_path || "",
        });
      })
      .catch((err) => {
        if (!active) return;
  ...
  const onUploadProof = async (file: File) => {
    ...
    try {
      ...
      toast.success("Bukti transfer berhasil diperbarui.", { title: "Berhasil" });
    } catch (err: any) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      await http.put(`/admin/donations/${donationId}`, payload);
      toast.success("Data donasi berhasil diperbarui.", { title: "Berhasil" });
      navigate(`${basePath}/donations/${donationId}`, { replace: true });
    } catch (err: any) {
      setErrors(normalizeErrors(err));
      toast.error("Gagal menyimpan perubahan donasi. Periksa kembali input Anda.", { title: "Gagal" });
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="mx-auto w-full max-w-7xl animate-fade-in space-y-6 pb-10">
        <div className="h-40 rounded-[32px] bg-slate-100 animate-pulse" />
        <div className="grid gap-8 lg:grid-cols-12">
          <div className="space-y-6 lg:col-span-8">
            <div className="h-80 rounded-[24px] bg-slate-100 animate-pulse" />
            <div className="h-60 rounded-[24px] bg-slate-100 animate-pulse" />
          </div>
          <div className="lg:col-span-4">
            <div className="h-72 rounded-[24px] bg-slate-100 animate-pulse" />
          </div>
        </div>
      </div>
    );
    ...
    <div className="mx-auto w-full max-w-7xl animate-fade-in space-y-8 pb-10">
        ...
        <div className="relative z-10 p-8 md:p-10">
          <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div className="space-y-4">
              <button
                onClick={() => navigate(`${basePath}/donations/${donationId}`)}
              ...
              <div>
                <div className="flex items-center gap-3">
                  <h1 className="font-heading text-3xl font-bold text-white md:text-5xl">Edit Data Donasi</h1>
                  <span className="rounded-full bg-white/20 px-3 py-1 text-xs font-mono font-bold text-white backdrop-blur">
                    {donationCode}
                  </span>
                </div>
                <p className="mt-2 text-lg text-emerald-100/90 font-medium max-w-2xl">
                  Perbarui rincian transaksi donasi manual, data donatur, dan bukti pembayaran.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        ...
        <div className="space-y-8 lg:col-span-8">
          ...
          {/* Bukti Pembayaran / Transfer (Opsional) */}
          <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="sticky top-8 space-y-6">
            <div className="rounded-[24px] border border-emerald-100 bg-white p-6 shadow-xl shadow-emerald-50/50">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="font-bold text-slate-900">Perbarui Data</h3>
                  <p className="text-xs text-slate-500">Simpan perubahan donasi</p>
                </div>
              </div>
...
export default AdminDonationManualEditPage;
