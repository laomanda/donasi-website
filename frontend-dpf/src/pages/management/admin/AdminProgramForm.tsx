...
export function AdminProgramForm({ mode, programId }: { mode: Mode; programId?: number }) {
  ...
  useEffect(() => {
    ...
    setErrors([]);
    http
      .get<EditorProgram>(`/admin/programs/${programId}`)
      .then((res) => {
        if (!active) return;
        const p = res.data;
  ...
  useEffect(() => {
    http
      .get("/admin/programs", { params: { per_page: 200 } })
      .then((res) => {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        await http.put(`/admin/programs/${programId}`, payload);
  ...
  const onDelete = async () => {
    ...
    setErrors([]);
    try {
      await http.delete(`/admin/programs/${programId}`);
