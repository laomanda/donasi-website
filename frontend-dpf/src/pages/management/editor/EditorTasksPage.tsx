...
export function EditorTasksPage() {
  ...
  const fetchTasks = async (
    ...
    const resolvedPerPage = nextPerPage ?? perPage;
  ...
  useEffect(() => {
    void fetchTasks(1, statusFilter, perPage);
  ...
  useEffect(() => {
    ...
    const refreshTasks = () => {
      ...
      void fetchTasks(pageRef.current, statusRef.current, perPageRef.current, { silent: true }).finally(
        () => {
      ...
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_220px] sm:items-start">
        <div className="min-w-0 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={["inline-flex items-center rounded-full px-3 py-1 text-[11px] font-bold ring-1", statusTone].join(" ")}>
              {statusLabel}
            </span>
          </div>
          {item.description ? (
        ...
        <div className="flex flex-wrap items-center gap-2 sm:flex-col sm:items-stretch">
          <button
            ...
            className="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 py-2 text-[11px] font-bold text-slate-700 shadow-sm transition hover:bg-slate-50"
          >
