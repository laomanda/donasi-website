...
export default function EditorGalleryDpfEditPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    let active = true;
    http.get<GalleryDpf>(`/editor/gallery-dpf/${galleryId}`)
      .then((response) => {
        if (!active) return;
        const item = response.data;
        setForm({ image: item.image ?? "", caption_id: item.caption_id ?? "", caption_en: item.caption_en ?? "", status: (item.status as GalleryDpfFormState["status"]) ?? "draft" });
      })
      .catch((error) => { if (active) setErrors(getErrorMessages(error, "Gagal memuat aktivitas DPF.")); })
  ...
  const onUpload = async (file: File) => {
    setUploadError(null);
    if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) { setUploadError("Format gambar harus JPG, JPEG, PNG, atau WebP."); return; }
    if (file.size > 10 * 1024 * 1024) { setUploadError("Ukuran gambar maksimal 10 MB."); return; }
  ...
  const onSubmit = async () => {
    ...
    setErrors([]);
    try {
      await http.put(`/editor/gallery-dpf/${galleryId}`, form);
      toast.success("Aktivitas DPF berhasil diperbarui.", { title: "Berhasil" });
      navigate("/editor/gallery-dpf", { replace: true });
    } catch (error) {
      setErrors(getErrorMessages(error, "Gagal memperbarui aktivitas DPF."));
    } finally {
      setSaving(false);
    }
  };

  if (loading) return <div className="mx-auto w-full max-w-6xl animate-pulse rounded-[28px] border border-slate-200 bg-white p-10 text-sm font-semibold text-slate-400">Memuat aktivitas DPF...</div>;
