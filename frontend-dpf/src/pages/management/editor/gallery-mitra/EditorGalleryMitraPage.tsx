...
export default function EditorGalleryMitraPage() {
  ...
  const handleDelete = async (item: GalleryMitra) => {
    setDeletingId(item.id);
    try {
      await http.delete(`/editor/gallery-mitra/${item.id}`);
      ...
      toast.error("Gagal menghapus aktivitas mitra.", { title: "Gagal" });
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const onDeleteSelected = async () => {
    ...
    try {
      ...
      const result = await runWithConcurrency(idsToDelete, 4, async (id) => {
        await http.delete(`/editor/gallery-mitra/${id}`);
      });
      ...
      if (result.failed.length > 0) {
        ...
        toast.success(`Berhasil menghapus ${result.succeeded.length} aktivitas mitra.`, {
          title: "Berhasil",
        });
        selection.clear();
      }
