...
import { emptyGalleryMitraForm, galleryMitraFolder, type GalleryMitraFormState } from "@/components/management/editor/gallery-mitra/GalleryMitraTypes";
...
export default function EditorGalleryMitraCreatePage() {
  ...
  const onSubmit = async () => {
    ...
    setErrors([]);
    try {
      await http.post('/editor/gallery-mitra', form);
      toast.success('Aktivitas mitra berhasil dibuat.', { title: 'Berhasil' });
      navigate('/editor/gallery-mitra', { replace: true });
    } catch (error) {
      setErrors(getErrorMessages(error, 'Gagal membuat aktivitas mitra.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 pb-20">
      <EditorGalleryMitraFormHeader mode="create" saving={saving} uploading={uploading} canSubmit={!saving && !uploading} onBack={() => navigate('/editor/gallery-mitra')} onSubmit={onSubmit} />
